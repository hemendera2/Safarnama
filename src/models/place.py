...
from sqlalchemy import String, Integer, Float, ForeignKey, Text, Table, DateTime, func, JSON
...
class Place(db.Model):
    ...
    village_id: Mapped[str] = mapped_column(ForeignKey("villages.id"), nullable=True, index=True)
    
    # Classification
    category_id: Mapped[str] = mapped_column(ForeignKey("categories.id"), nullable=False, index=True)
    subcategory_id: Mapped[str] = mapped_column(ForeignKey("subcategories.id"), nullable=True, index=True)
    ...
    difficulty_level: Mapped[str] = mapped_column(String(50), nullable=True) # Easy, Moderate, Hard
...
class PlaceImage(db.Model):
    ...
    image_type: Mapped[str] = mapped_column(String(50), default="gallery") # hero, gallery, drone, 360
