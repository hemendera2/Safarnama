...
class ImportService:
    ...
    def import_from_csv(file_path, source="Manual Import"):
        ...
        for index, row in df.iterrows():
            try:
                ...
                existing = Place.query.filter(
                    ...
                    Place.latitude.between(float(data['latitude'])-0.01, float(data['latitude'])+0.01)
                ).first()
                ...
                place = Place(
                    ...
                    confidence_score=data.get('confidence', 0.8) # Default to 0.8 for new imports
                )
        ...
        try:
            db.session.commit()
            logger.info(f"Import complete: {success_count} success, {duplicate_count} skipped, {len(errors)} failed.")
        except Exception as e:
            db.session.rollback()
            logger.error("Transaction failed", error=str(e))
            
        return {"success": success_count, "duplicates": duplicate_count, "errors": errors}
