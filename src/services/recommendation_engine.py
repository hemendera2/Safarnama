        ...
        for place in places:
            ...
            if place.intelligence:
                ...
                if context.get("interest") == "photography":
                    ...
                    if intel.photography > 0.8: explanations.append("Exceptional photography potential")
                
                if context.get("vibe") == "adventure":
                    score += intel.adventure * 0.3
                    if intel.adventure > 0.8: explanations.append("Perfect for adventure seekers")

            # 2. Seasonality Signal (0.3 weight)
            current_season = context.get("season", "monsoon").lower()
            if place.intelligence:
                season_score = getattr(place.intelligence, f"{current_season}_value", 0.5)
                score += season_score * 0.3
                if season_score > 0.8: explanations.append(f"Top-rated for {current_season.capitalize()}")
            ...
            if rel_count > 0:
                ...
                explanations.append(f"Part of a rich travel circuit")
            ...
            if place.crowd_factor <= 2: explanations.append("Low crowd/Peaceful")
